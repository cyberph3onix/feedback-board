...
import { FeedbackBoardPrivateState } from 'feedback-board-contract';
import { inMemoryPrivateStateProvider } from '../in-memory-private-state-provider';
import { NetworkId } from '@midnight-ntwrk/midnight-js-network-id';
import type { UnboundTransaction } from '@midnight-ntwrk/midnight-js-types';

/**
 * An in-progress anonymous feedback board deployment.
 */
...
/**
 * A deployed anonymous feedback board deployment.
 */
export interface DeployedBoardDeployment {
  readonly status: 'deployed';

  /**
   * The {@link DeployedFeedbackBoardAPI} instance when connected to an on network anonymous feedback board contract.
   */
  readonly api: DeployedFeedbackBoardAPI;
}

/**
 * A failed anonymous feedback board deployment.
 */
...
export interface DeployedBoardAPIProvider {
  ...
  /**
   * Joins or deploys an anonymous feedback board contract.
   *
   * @param contractAddress An optional contract address to use when resolving.
   * @returns An observable board deployment.
   *
   * @remarks
   * For a given `contractAddress`, the method will attempt to find and join the identified anonymous feedback board
...
/**
 * A {@link DeployedBoardAPIProvider} that manages anonymous feedback board deployments in a browser setting.
 *
 * @remarks
...
const initializeProviders = async (logger: Logger): Promise<FeedbackBoardProviders> => {
  ...
  const zkConfigPath = window.location.origin; // '../../../contract/src/managed/feedback-board';
